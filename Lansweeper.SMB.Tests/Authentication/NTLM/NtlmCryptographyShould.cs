...
internal class NtlmCryptographyShould
{
    [Test]
    ...
    public void LMv1HashTestEmptyPassword()
    {
        ...
        byte[] expected = [0xaa, 0xd3, 0xb4, 0x35, 0xb5, 0x14, 0x04, 0xee, 0xaa, 0xd3, 0xb4, 0x35, 0xb5, 0x14, 0x04, 0xee];
        hash.Should().Equal(expected);
    }
    ...
    public void NTLMv2ResponseTest()
    {
        ...
        NtlmV2ClientChallenge clientChallengeStructure = new(time, clientChallenge, "Domain", "Server");
        ...
        clientNTProof.Should().Equal(expectedNTProof);
    }
    ...
    public void NTLMv2ChallengeMessageTest()
    {
        ...
        expected.Should().Equal(messageBytes);
    }
    ...
    public void NTLMv2AuthenticateMessageTest()
    {
        ...
        AuthenticateMessage cmp = new(expected);
        ...
        messageBytes.Should().Equal(cmp.GetBytes());
    }
    ...
    public void TestLMMIC()
    {
        ...
        AuthenticateMessage authenticateMessage = new(type3);
        byte[] sessionBaseKey = MD4.GetByteHashFromBytes(NtlmCryptography.NTOWFv1(password));
    ...
    public void TestNTLMv2KeyExchangeMIC()
    {
        ...
        byte[] serverChallenge = new ChallengeMessage(type2).ServerChallenge;
        AuthenticateMessage authenticateMessage = new(type3);
        byte[] ntProofStr = ByteReader.ReadBytes(authenticateMessage.NtChallengeResponse.AsSpan(), 16);
        byte[] sessionBaseKey = HMACMD5.HashData(responseKeyNT, ntProofStr);
    ...
    {
        // Arrange
        byte[] exportedSessionKey = "UUUUUUUUUUUUUUUU"u8.ToArray();
        byte[] expected = [0x47, 0x88, 0xdc, 0x86, 0x1b, 0x47, 0x82, 0xf3, 0x5d, 0x43, 0xfd, 0x98, 0xfe, 0x1a, 0x2d, 0x39];

        // Act
        byte[] signKey = NtlmCryptography.ComputeClientSignKey(exportedSessionKey);

        // Assert
        signKey.Should().Equal(expected);
    }
    ...
    public void Test_ComputeClientSealKey()
    {
        // Arrange
        byte[] exportedSessionKey = "UUUUUUUUUUUUUUUU"u8.ToArray();
        byte[] expected = [0x59, 0xf6, 0x00, 0x97, 0x3c, 0xc4, 0x96, 0x0a, 0x25, 0x48, 0x0a, 0x7c, 0x19, 0x6e, 0x4c, 0x58];

        // Act
        byte[] sealKey = NtlmCryptography.ComputeClientSealKey(exportedSessionKey);

        // Assert
        sealKey.Should().Equal(expected);
    }
    ...
    {
        // Arrange
        byte[] exportedSessionKey = [0xBE, 0xC7, 0x33, 0xF6, 0x23, 0xB1, 0x2B, 0x98, 0xD4, 0x21, 0xFF, 0xCD, 0xD5, 0x42, 0xE3, 0xA2];
        byte[] mechListMicBytes = [0x30, 0x0c, 0x06, 0x0a, 0x2b, 0x06, 0x01, 0x04, 0x01, 0x82, 0x37, 0x02, 0x02, 0x0a];
        byte[] expectedMic = [0x01, 0x00, 0x00, 0x00, 0xa1, 0xa6, 0x7b, 0x80, 0x35, 0xc1, 0x76, 0xd3, 0x00, 0x00, 0x00, 0x00];
